...
public class AProtester extends PApplet {
    ...
    public void displayScene() {
        image(game_state.background, win_x, win_y);
    }
    ...
    public void lightUp() {
        ...
        game_state.background = loadImage(scenes.get(game_state.cur_scene).background);
        game_state.background.resize(win_width, win_height);
        game_state.background.loadPixels();
        game_state.background.updatePixels();
    ...
    class Animation {
        ...
        public void display(int frame, float xpos, float ypos, int width, int height) {
            // println(xpos + " " + ypos + " " + my_width + " " + my_height);
            image(images[frame], xpos, ypos, width, height);
        }

        public void resize(int my_width, int my_height) {
            for (int i = 0; i < image_count; i++) {
                images[i].resize(my_width, my_height);
                images[i].loadPixels();
